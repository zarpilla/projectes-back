  ...
  if [ ! -f "$V5_CONFIG_FILE" ]; then
    ...
    node -e '
      ...
      app.env = {
        ...
        // Encrypts the certificate passphrases and integration API keys at
        // rest (src/services/secret-crypto.js). Generated ONCE with the rest:
        // change it later and every stored secret becomes unreadable.
        SECRETS_KEY: require("crypto").randomBytes(32).toString("hex"),
      };
